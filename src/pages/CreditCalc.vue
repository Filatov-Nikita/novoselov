<template>
  <app-page class="page-pb">
    <div class="wrapper">
      <h1 class="h1 tw-max-w-[340px] tw-mb-20 md:tw-max-w-full md:tw-mb-40">
      ...
      <section>
        <div
          ...
          <button
            class="tw-ml-10"
            @mouseenter="bankPopup = true"
            @mouseleave="bankPopup = false">
            <AppIcon
              name="attention"
              size="18px"
...
export default {
  ...
  data() {
    return {
      ...
      caption:
        "Расчет кредита является предварительным, не является публичной офертой. Получите подробную информацию у специалиста отдела продаж.",
      partners: [
        {
          alt: "Сбер",
  ...
  methods: {
    calculate(price, pay, percent, years) {
      ...
      const r =
        this.creditSum * ((i * Math.pow(1 + i, n)) / (Math.pow(1 + i, n) - 1));
